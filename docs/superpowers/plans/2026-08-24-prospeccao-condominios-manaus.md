...
`basedosdados.br_me_cnpj` (Tarefa 6, implantação). Correções feitas nessa
validação, em relação à primeira versão:
...
def build_client(credentials_json: str):
    ...
    credentials = service_account.Credentials.from_service_account_info(
        info, scopes=["https://www.googleapis.com/auth/bigquery"]  # readonly nao basta: rodar query cria um job (escrita)
    )
